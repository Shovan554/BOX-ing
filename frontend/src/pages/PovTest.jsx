...
const PovTest = () => {
  ...
  // Listen for 'w' key to trigger animations
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (e.key.toLowerCase() === 'w') {
        // Trigger jab/hook on both
        if (playerRef.current) playerRef.current.playAction('left hook');
    ...
    <div className="w-full h-full relative bg-[#050505]">
      ...
      <div className="absolute top-0 left-0 w-full p-8 z-50 flex justify-between items-start pointer-events-none">
        ...
        <div className="text-right">
          <h2 className="text-white font-black italic tracking-tighter text-2xl uppercase">POV Sync Test</h2>
          <p className="text-red-500/60 font-mono text-[10px] tracking-widest uppercase mt-1">Experimental Perspective Engine v1.0</p>
        </div>
      </div>
      ...
      <div className="w-full h-full">
        <Canvas shadows>
          ...
          <Suspense fallback={null}>
            ...
            {/* Player: Facing away from the camera (Third Person POV) */}
      ...
      <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex gap-8 z-50 pointer-events-none opacity-40">
        <div className="flex flex-col items-center gap-2">
          ...
          <span className="text-xs font-bold text-white uppercase italic">Player (POV)</span>
        </div>
